// lib/agents/debate-generator-agent.js
import { z } from 'zod';
import BaseAgent from './base-agent';

class DebateGeneratorAgent extends BaseAgent {
  constructor(apiKey) {
    super('Debate Generator', 'Topic Analysis & Argument Creation', apiKey);
  }

  // Main method: Generate debate topic from curated news
  ...
  // Create comprehensive prompt for debate generation
...
Generate a debate topic that will spark meaningful discussion and engagement.`;
  ...
  // Define schema for structured debate generation using Zod
  getDebateSchema() {
    return z.object({
      title: z.string().describe('A clear, engaging debate question (max 100 characters)'),
      description: z.string().describe('Brief context about the issue (max 200 characters)'),
      category: z.string().describe('Category like politics/crypto/technology/economy/etc'),
      debatePoints: z.object({
        Support: z.array(z.string()).describe('3-4 compelling arguments supporting one side'),
        Oppose: z.array(z.string()).describe('3-4 compelling arguments opposing the same side')
      }).describe('Two opposing sides with key arguments'),
      complexity: z.enum(['simple', 'moderate', 'complex']).describe('Complexity level of the debate topic'),
      controversyLevel: z.enum(['low', 'medium', 'high']).describe('Level of controversy expected'),
      targetAudience: z.enum(['general', 'informed', 'expert']).describe('Target audience for the debate')
    });
  }

  // Generate additional debate variations
  ...
  // Create prompt for debate variations
...
Provide specific feedback and suggestions for improvement.`;
  ...
  // Define schema for quality analysis
...
export default DebateGeneratorAgent;
