...
class SerperNewsService {
  ...
  constructor() {
    ...
    if (!this.apiKey) {
      console.warn('[Serper News Service] SERPER_API_KEY not configured in environment');
    } else {
      console.log('[Serper News Service] API key configured successfully');
    }
  }
  ...
  private generateDynamicQuery(): string {
    ...
    const keywordGroups = {
      ...
      crypto: ['bitcoin', 'ethereum', 'cryptocurrency', 'blockchain', 'crypto market', 'digital currency']
    };
